...
def _get_layers_from_nex(gpkg: Path, nexus: pd.Series) -> Mapping[str, gpd.GeoDataFrame]:
    ...
    """
    nex = gpd.read_file(gpkg, layer='nexus')
    nex = nex.query('id in @nexus')
    pois = gpd.read_file(gpkg, layer='pois')
    pois = pois.query('nex_id in @nexus')
    hydro = gpd.read_file(gpkg, layer='hydrolocations')
...
def _get_layers_from_flowpath(gpkg: Path, flowpaths: pd.Series)-> Mapping[str, gpd.GeoDataFrame]:
    ...
    """
    attrs = gpd.read_file(gpkg, layer='flowpath-attributes')
    attrs = attrs.query('id in @flowpaths')
    attrs_ml = gpd.read_file(gpkg, layer='flowpath-attributes-ml')
...
def _get_layers_from_poi(gpkg: Path, pois: pd.Series)->Mapping[str, gpd.GeoDataFrame]:
    ...
    lakes = gpd.read_file(gpkg, layer='lakes')
...
def get_sub_layers(gpkg: Path, mainstem: Iterable[str], ghost:Optional[str]=None)->Mapping[str, gpd.GeoDataFrame]:
    ...
    """
    flowpaths = gpd.read_file(gpkg, layer='flowpaths').set_index('id')
    divides = gpd.read_file(gpkg, layer='divides')
    divide_attrs = gpd.read_file(gpkg, layer='divide-attributes')
    network = gpd.read_file(gpkg, layer='network')
...
if __name__ == "__main__":
    ...
    network = gpd.read_file(_to_open, layer='network')
    flowpaths = gpd.read_file(_to_open, layer='flowpaths').set_index('id')
    if args.plot_wide:
        divides = gpd.read_file(_to_open, layer='divides')
    ...
    # Prepare list to collect coverage data for CSV
    coverage_data = []
    ...
    if coverage_data:
        ...
        coverage_df.to_csv(csv_filename, index=False)
